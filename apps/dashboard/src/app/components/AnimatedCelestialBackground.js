...
exports.AnimatedCelestialBackground = AnimatedCelestialBackground;
...
function AnimatedCelestialBackground({ scannerCount = 5, isScanning = false, hasOpportunity = false, isTradeExecuting = false, liveTokenPrices = {}, floatingWords = DEFAULT_FLOATING_WORDS, }) {
    const canvasRef = (0, react_1.useRef)(null);
    const wordsCanvasRef = (0, react_1.useRef)(null);
    const stateRef = (0, react_1.useRef)({ scannerCount, isScanning, hasOpportunity, isTradeExecuting });
    const interactionRef = (0, react_1.useRef)({ hoverX: 0.5, hoverY: 0.5, scroll: 0 });
    const marketHeatRef = (0, react_1.useRef)([]);
    const floatingWordStateRef = (0, react_1.useRef)([]);
    const floatingItems = (0, react_1.useMemo)(() => {
    ...
    (0, react_1.useEffect)(() => {
        ...
        const drawLine = (a, b, alpha, width, color) => {
        ...
        const render = (t) => {
            ...
            const baseGlow = context.createRadialGradient(width * 0.5, height * 0.48, 0, width * 0.5, height * 0.48, Math.min(width, height) * 1.08);
            ...
            context.fillRect(0, 0, width, height);
            const champagneBeam = context.createRadialGradient(width * 0.62, height * 0.35, 0, width * 0.62, height * 0.35, Math.min(width, height) * 0.46);
            ...
            context.bezierCurveTo(width * 0.32, height * 0.58, width * 0.56, height * 0.64, width * 0.96, height * 0.28);
            context.stroke();
            ...
            context.bezierCurveTo(width * 0.30, height * 0.54, width * 0.64, height * 0.42, width * 0.97, height * 0.70);
            context.stroke();
            const finalGlow = context.createRadialGradient(width * 0.5, height * 0.5, 30, width * 0.5, height * 0.5, Math.min(width, height) * 0.78);
    ...
    return (<div className="pointer-events-none fixed inset-0 z-0 overflow-hidden" aria-hidden="true">
      ...
      <div style={{
            ...
            pointerEvents: "none",
            zIndex: 3,
            background: "radial-gradient(circle at 18% 82%, rgba(22, 52, 70, 0.18), transparent 34%), linear-gradient(90deg, rgba(0,0,0,0.08), transparent 30%, transparent 100%)",
        }}/>
    </div>);
}
