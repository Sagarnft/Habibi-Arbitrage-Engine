...
import type { QuoteResult } from "../../aggregator/types.js";
import { initializeTokenRegistry } from "../../tokens/init.js";
import { tokenRegistry } from "../../tokens/registry.js";

function getTokenSymbol(chain: ChainName, address: `0x${string}`): string {
...
function getMinNetProfitBps(): bigint {
...
function getWrappedNativeAddress(chain: ChainName): `0x${string}` | null {
  initializeTokenRegistry();
...
async function getGasCostInAssetUnits(
  chain: ChainName,
  tokenIn: `0x${string}`,
  gasCostWei: bigint,
): Promise<bigint> {
...
function estimateRouteMetrics(
  ...
  const gasImpactBps = routeSize > 0n
    ? Number((gasCost * 10_000n) / routeSize)
    : 0;
...
function getReserveQualityScore(amountIn: bigint, reserveIn: bigint, reserveOut: bigint): number {
  if (amountIn <= 0n || reserveIn <= 0n || reserveOut <= 0n) {
    return 0;
  }

  const usageBps = Number((amountIn * 10_000n) / reserveIn);
  const outputDepthBps = Number((amountIn * 10_000n) / reserveOut);
...
function getFreshnessPenalty(opportunity: Opportunity): number {
  ...
  const quotePenalty = Number.isFinite(quoteAgeMs) && quoteAgeMs > 0
   ? Math.min(20, Math.round(quoteAgeMs / 15_000))
   : 0;
...
function compareQuoteCandidates(left: QuoteResult, right: QuoteResult): number {
...
export async function findOpportunity(
  ...
  for (const buyQuote of liveQuotes) {
    ...
    const minimumNetProfit = (amountIn * MIN_NET_PROFIT_BPS) / 10_000n;
...
export async function findTriangleOpportunity(
  chain: ChainName,
  tokenA: `0x${string}`,
  tokenB: `0x${string}`,
  tokenC: `0x${string}`,
