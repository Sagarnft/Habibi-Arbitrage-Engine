...
export function Sidebar({ sections, onItemClick, activeLabel, profileName, profileAvatarDataUrl }: SidebarProps) {
  ...
  return (
    <aside className="flex h-full w-64 flex-col overflow-y-auto border-r border-slate-200/10 bg-gradient-to-b from-slate-950/92 via-slate-900/78 to-slate-950/92 p-4 backdrop-blur-xl shadow-[0_18px_40px_rgba(0,0,0,0.35)]">
      {/* Logo */}
      <div className="mb-8 flex items-center gap-2">
        <div className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-lg bg-gradient-to-br from-amber-300 to-amber-600 text-sm font-bold text-slate-950 shadow-[0_0_18px_rgba(200,155,60,0.18)]">
          {profileAvatarDataUrl ? (
            <img src={profileAvatarDataUrl} alt="Profile logo" className="h-full w-full object-cover" />
          ) : (
            profileInitials
          )}
        </div>
              ...
              {section.items.map((item, itemIdx) => (
                <li key={itemIdx}>
                  <button
                    ...
                    className={`flex w-full items-center gap-2.5 rounded-lg border px-3 py-2.5 text-sm font-medium transition ${activeLabel === item.label ? "border-amber-300/30 bg-amber-100/20 text-amber-100 shadow-[0_0_18px_rgba(200,155,60,0.12)]" : "border-transparent text-slate-300 hover:border-amber-300/20 hover:bg-white/5 hover:text-amber-200 active:bg-amber-300/10"}`}
                  >
