...
class HealthMonitor {
    status = new Map();
    failureLimit = Number(process.env.RPC_FAILURE_LIMIT ?? 3);
    cooldownMs = Number(process.env.RPC_FAILURE_COOLDOWN_MS ?? 120_000);
    async check(chain) {
        const urls = [...rpcManager.getCandidateUrls(chain)];
        const previous = this.status.get(chain);
        for (const url of urls) {
            const client = rpcManager.getClientForUrl(chain, url);
    ...
    setStatus(chain, health) {
        ...
        this.status.set(chain, {
            ...health,
            failureCount,
            cooldownUntil: health.cooldownUntil ?? (failureCount >= this.failureLimit
                ? Date.now() + this.cooldownMs
                : previous?.cooldownUntil ?? 0),
        });
    }
