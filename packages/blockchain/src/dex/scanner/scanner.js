...
import { findOpportunity, findTriangleOpportunity } from "./opportunity.js";
...
function getMinimumTrialAmount(baseAmountIn) {
    ...
    const minimum = (baseAmountIn * BigInt(normalizedBps)) / 10000n;
...
function buildTrialAmounts(baseAmountIn) {
    const trialAmounts = [];
...
function getChainScanScore(chain) {
...
function isChainEnabled(chain) {
    return !healthMonitor.isTemporarilyDisabled(chain);
}
export async function scan(request, pairCandidates) {
    ...
    const pairs = pairCandidates?.length
        ? [...pairCandidates]
            ...
            .filter((candidate) => !canUseHealthyFilter || healthyChains.has(candidate.chain))
            .map((candidate) => ({
        ...
        : buildTokenPairs(request.tokens).map((pair) => ({
            chain: request.chain,
    ...
    let failedPairs = 0;
    const dexActivity = new Map();
    initializeTokenRegistry();
    const ensureDexActivity = (dex) => {
        const normalizedDex = dex.toLowerCase().replace(/[^a-z0-9]+/g, "-");
        const current = dexActivity.get(normalizedDex) ?? { quotes: 0, pairs: new Set(), opportunities: 0 };
    ...
    const tasks = pairs.map((pair) => async () => {
        try {
            ...
            const amountIn = buildAmountIn(pair.chain, pair.tokenIn);
            if (amountIn <= 0n) {
                return null;
            }
            ...
            for (const trialAmountIn of trialAmounts) {
                const quotes = await getQuotes(pair.chain, pair.tokenIn, pair.tokenOut, trialAmountIn);
                for (const quote of quotes) {
                    const currentDex = ensureDexActivity(quote.dex);
                    currentDex.quotes += 1;
                    currentDex.pairs.add(pairKey);
                }
                const discoveredOpportunity = await findOpportunity(pair.chain, pair.tokenIn, pair.tokenOut, trialAmountIn, quotes);
    ...
    const triangleTasks = triangleScansEnabled
        ? Object.entries(TRIANGLE_SYMBOLS).flatMap(([chain, triangles]) => triangles.map(([symbolA, symbolB, symbolC]) => async () => {
            try {
                ...
                const amountIn = buildAmountIn(chain, tokenA.address);
                if (amountIn <= 0n) {
                    return null;
                }
                return await findTriangleOpportunity(chain, tokenA.address, tokenB.address, tokenC.address, amountIn);
            }
            catch {
                failedPairs++;
                return null;
        ...
        : [];
    const results = await runWithConcurrency(tasks, SCAN_CONCURRENCY);
    ...
    for (const opportunity of [...results, ...triangleResults]) {
        ...
        const risk = checkOpportunityRisk(opportunity);
        ...
        const passesFallbackQualityGate = (opportunity.profitable
