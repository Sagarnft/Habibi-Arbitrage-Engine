...
const sanitizeDashboardPayload = (payload) => {
...
const mergeOperationalPayload = (payload, opsMetricsPayload, settlementQueuePayload, rolloutPayload) => {
    const nextPayload = { ...payload };
    if (opsMetricsPayload && typeof opsMetricsPayload === "object") {
        const ops = opsMetricsPayload;
...
const fetchJsonFromUpstream = async (upstreamBaseUrl, path, signal) => {
    try {
...
async function GET() {
    const upstreamBaseUrl = (0, upstream_1.resolveUpstreamBaseUrl)();
    ...
    try {
        ...
        if (!response.ok) {
            return server_1.NextResponse.json(EMPTY_DASHBOARD_STATE, { status: 200 });
        }
        ...
        const normalizedPayload = payload && typeof payload === "object"
            ? {
                ...
                ...sanitizeDashboardPayload(mergeOperationalPayload(payload, opsMetricsPayload, settlementQueuePayload, rolloutPayload)),
            }
            : EMPTY_DASHBOARD_STATE;
        return server_1.NextResponse.json(normalizedPayload, { status: 200 });
    }
    catch {
        return server_1.NextResponse.json(EMPTY_DASHBOARD_STATE, { status: 200 });
    }
