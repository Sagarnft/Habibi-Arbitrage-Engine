...
import { appendSettlementRecord, getExecutionStoreMeta, getRolloutGovernanceState, getSettlementRecords, readExecutionState, writeReadinessGateHistory, writeRecoveryState, writeRolloutGovernanceState, writeSettlementQueue } from "./executionStore.js";
const app = express();
...
async function primeLiveDexActivity() {
    const probePairs = [
        {
            chain: "ethereum",
...
const DEFAULT_MIN_GAS_RESERVE_NATIVE = {
    ethereum: 0.01,
    arbitrum: 0.007,
    base: 0.007,
    optimism: 0.007,
...
function parsePolicyNumber(rawValue, fallback, min, max) {
...
function buildCapitalPolicySnapshot() {
    ...
    const minGasReserveNative = Object.fromEntries(Object.entries(DEFAULT_MIN_GAS_RESERVE_NATIVE).map(([chain, fallback]) => [
        chain,
...
function evaluateCapitalPolicyGuard(request) {
    ...
    const emergencyReserveUsd = Math.max(policy.emergencyReserveFloorUsd, walletUsdtBalance * policy.emergencyAllocationShare);
...
function normalizeDexLabel(value) {
...
export function serializeForJson(value) {
    ...
    if (value && typeof value === "object") {
        ...
        return Object.fromEntries(Object.entries(value).map(([key, nestedValue]) => [key, serializeForJson(nestedValue)]));
    }
    return value;
}
export function buildDashboardScanCandidates() {
    const candidates = [];
    const seen = new Set();
    const routesByChain = [
...
async function getLiveScanResult() {
    if (latestScanResult) {
...
const executionIntentRegistry = new Map();
let executionIntentHistory = [];
const EXECUTION_PREPARE_COOLDOWN_MS = Math.max(0, Number(process.env.EXECUTION_PREPARE_COOLDOWN_MS ?? 1500));
const EXECUTION_CONFIRM_COOLDOWN_MS = Math.max(0, Number(process.env.EXECUTION_CONFIRM_COOLDOWN_MS ?? 2500));
const RECOVERY_WORKER_ENABLED = process.env.RECOVERY_WORKER_ENABLED !== "false";
const RECOVERY_WORKER_INTERVAL_MS = Math.max(5000, Number(process.env.RECOVERY_WORKER_INTERVAL_MS ?? 15000));
const RECOVERY_WORKER_MAX_ATTEMPTS = Math.max(1, Number(process.env.RECOVERY_WORKER_MAX_ATTEMPTS ?? 5));
const RECOVERY_WORKER_BACKOFF_MS = Math.max(1000, Number(process.env.RECOVERY_WORKER_BACKOFF_MS ?? 30000));
const RECOVERY_WORKER_MAX_BACKOFF_MS = Math.max(5000, Number(process.env.RECOVERY_WORKER_MAX_BACKOFF_MS ?? 5 * 60_000));
...
const SETTLEMENT_WORKER_INTERVAL_MS = Math.max(5000, Number(process.env.SETTLEMENT_WORKER_INTERVAL_MS ?? 20000));
const SETTLEMENT_WORKER_MAX_ATTEMPTS = Math.max(1, Number(process.env.SETTLEMENT_WORKER_MAX_ATTEMPTS ?? 8));
const SETTLEMENT_WORKER_BACKOFF_MS = Math.max(2000, Number(process.env.SETTLEMENT_WORKER_BACKOFF_MS ?? 45000));
const SETTLEMENT_WORKER_MAX_BACKOFF_MS = Math.max(10000, Number(process.env.SETTLEMENT_WORKER_MAX_BACKOFF_MS ?? 10 * 60_000));
const SETTLEMENT_WORKER_MIN_PROFIT_BPS = Math.max(-500, Math.min(2000, Number(process.env.SETTLEMENT_WORKER_MIN_PROFIT_BPS ?? 35)));
...
const ROLLOUT_PROMOTION_COOLDOWN_MS = Math.max(60000, Number(process.env.ROLLOUT_PROMOTION_COOLDOWN_MS ?? 10 * 60_000));
const ROLLOUT_DEMOTION_COOLDOWN_MS = Math.max(30000, Number(process.env.ROLLOUT_DEMOTION_COOLDOWN_MS ?? 3 * 60_000));
const CANARY_VALIDATION_CHAIN = CHAIN_LIST.includes((process.env.CANARY_VALIDATION_CHAIN ?? "arbitrum"))
    ? process.env.CANARY_VALIDATION_CHAIN
    : "arbitrum";
...
const RELAY_REQUIRED_CHAINS_DEFAULT = ["arbitrum", "bnb", "base"];
const RELAY_REQUIRED_CHAINS = (() => {
    const parsed = String(process.env.RELAY_REQUIRED_CHAINS ?? "")
        .split(",")
        .map((value) => value.trim().toLowerCase())
        .filter((value) => CHAIN_LIST.includes(value));
    return parsed.length > 0 ? parsed : RELAY_REQUIRED_CHAINS_DEFAULT;
})();
const executionThrottleRegistry = new Map();
const recoveryTicketRegistry = new Map();
let recoveryTicketHistory = [];
const settlementQueueRegistry = new Map();
const rolloutGovernanceRegistry = new Map();
let readinessGateHistory = [];
...
const normalizePersistedChain = (value) => (CHAIN_LIST.includes(value) ? value : "arbitrum");
...
for (const row of getRolloutGovernanceState()) {
    ...
    const currentStage = row.currentStage === "scale" || row.currentStage === "canary" ? row.currentStage : "blocked";
    const manualOverrideStage = row.manualOverrideStage === "scale" || row.manualOverrideStage === "canary" || row.manualOverrideStage === "blocked"
...
function formatTokenUsd(value, decimals = 18) {
...
function toUsdAmount(amount, decimals = 18) {
    const scale = 10 ** Math.min(decimals, 18);
    return Number(amount) / scale;
}
export function isExecutionApiAuthorized(req, res) {
...
function isExecutionSignerReady(res) {
...
function getExecutionThrottleKey(walletAddress, chain) {
    return `${walletAddress.toLowerCase()}::${chain}`;
}
function getExecutionThrottleState(walletAddress, chain) {
...
function formatLiquidityState(slippageBps, gasImpactBps) {
...
export function recordExecutedTrade(request) {
...
function snapshotRecoveryTicket(ticket) {
    return {
        ...ticket,
        route: ticket.route ? { ...ticket.route } : undefined,
    };
...
function persistRecoveryState() {
    writeRecoveryState(Array.from(recoveryTicketRegistry.values()).map((ticket) => ({
        id: ticket.id,
        chain: ticket.chain,
        walletAddress: ticket.walletAddress.toLowerCase(),
...
function recordRecoveryTicketHistory(ticket) {
...
function updateRecoveryTicket(ticketId, patch) {
...
function computeRecoveryRetryBackoffMs(attempt) {
...
function persistSettlementQueueState() {
    writeSettlementQueue(Array.from(settlementQueueRegistry.values()).map((item) => ({
        id: item.id,
        txHash: item.txHash,
        chain: item.chain,
        walletAddress: item.walletAddress.toLowerCase(),
...
function updateSettlementQueueItem(queueId, patch) {
...
function resolveNativeUsdPriceForChain(chain) {
...
function computeSettlementRetryBackoffMs(attempt) {
    const exponent = Math.max(0, attempt - 1);
...
export function buildCanaryValidationStatus(input) {
    ...
    const queueByHash = new Map(Array.from(settlementQueueRegistry.values()).map((item) => [item.txHash.toLowerCase(), item]));
...
function observedStageIsNotBlocked(stage) {
...
export function getReadinessGateHistory(windowSize = 5) {
    return readinessGateHistory.slice(0, Math.max(1, windowSize));
}
export function resetReadinessGateHistoryForTests() {
...
export function buildLiveCostTuningSnapshot(input) {
    ...
    const gasCostBufferUsd = Math.min(LIVE_COST_TUNING_MAX_GAS_BUFFER_USD, Math.max(0, avgGasCostUsd * 0.25));
    const slippageMultiplier = Math.min(LIVE_COST_TUNING_MAX_SLIPPAGE_MULTIPLIER, Math.max(1, 1 + (avgSlippageBps / 400)));
    const sizingPenaltyMultiplier = Math.max(LIVE_COST_TUNING_MIN_SIZING_MULTIPLIER, Math.min(1, 1 - (lossRate * 0.35) - (avgSlippageBps / 2000)));
    return {
...
export function resolveExecutionIntent(intentId) {
...
export function retireExecutionIntent(intentId, status, txHash) {
    const intent = executionIntentRegistry.get(intentId);
    if (!intent) {
...
async function isReceiptConfirmed(txHash, chain = "bnb") {
    try {
...
function buildApprovalTransaction(tokenAddress, spender) {
    return {
        to: tokenAddress,
...
function applySlippage(amountOut, slippageBps) {
    ...
    return (amountOut * BigInt(10_000 - boundedBps)) / 10000n;
}
export function resolvePrivateRelayRpcUrl(chain) {
...
function getPrivateRelayRpcUrlCandidates(chain) {
    const chainKey = chain.toUpperCase();
    return [
...
function normalizePrivateRelayRpcUrl(candidate) {
...
function getPrivateRelayRpcUrlStatus(chain) {
    const candidates = getPrivateRelayRpcUrlCandidates(chain);
...
function isHexRawTransaction(value) {
    return typeof value === "string" && /^0x[a-fA-F0-9]+$/.test(value) && value.length > 2;
}
export async function submitSignedTransactionsToPrivateRelay(chain, signedTransactions) {
...
function findTokenSymbol(chain, address) {
...
function formatRoutePair(chain, tokenIn, tokenOut) {
    return `${findTokenSymbol(chain, tokenIn)}/${findTokenSymbol(chain, tokenOut)}`;
}
async function getLatestExecutableOpportunity(chain) {
...
async function buildPreparedExecutionTransactions(request) {
    ...
    for (let index = 0; index < preparedPlan.steps.length; index += 1) {
        ...
        const quotedSwap = (await getQuotes(request.chain, step.tokenIn, step.tokenOut, currentAmountIn)).find((quote) => normalizeDexLabel(quote.dex) === normalizeDexLabel(step.dex));
        ...
        const minAmountOut = applySlippage(quotedSwap.amountOut, Number(request.slippageBps ?? 25));
        const swap = await adapter.buildSwapTransaction({
    ...
    if (effectiveUseFlashloan) {
        ...
        return {
            ...
            liveCostTuning,
            readinessGate,
            executionIntentId: createExecutionIntent({
                chain: request.chain,
                walletAddress: request.walletAddress,
                route: {
                    pair: formatRoutePair(request.chain, liveOpportunity.tokenIn, liveOpportunity.tokenOut),
            ...
            executionIntentExpiresAt: new Date(Date.now() + EXECUTION_INTENT_TTL_MS).toISOString(),
            route: {
                pair: formatRoutePair(request.chain, liveOpportunity.tokenIn, liveOpportunity.tokenOut),
                buyDex: liveOpportunity.buyDex,
                sellDex: liveOpportunity.sellDex,
...
async function isExecutableArbitrageTransaction(txHash, walletAddress, chain = "bnb") {
    try {
        const client = getClient(chain);
...
let rpcHealthRefreshInFlight = null;
...
async function refreshDashboardFromScan(scanResult) {
    ...
    dashboardState = {
        ...
        risk: {
            ...(dashboardState.risk ?? {}),
            ...(snapshot.risk ?? {}),
            reason: (snapshot.risk?.reason ?? dashboardState.risk?.reason),
        },
...
export async function buildDashboardSnapshot(input) {
    ...
    const protection = mevGuard.evaluate(Number(topOpportunity?.slippageBps ?? 0), topOpportunity?.netProfit ?? 0n);
    ...
    const configuredDexNames = Array.from(new Set(dexRegistry.getAll().map((adapter) => normalizeDexLabel(adapter.id ?? adapter.name)).filter(Boolean)));
    ...
    const dexActivityByName = scanDexActivity.reduce((accumulator, activity) => {
    ...
    const readyPairs = Math.max(scannedPairs - failedPairs, resolvedOpportunityRows.length, hasLiveOpportunities ? liveOpportunities.length : 0);
    ...
    const dexAnalytics = dexNames.map((name) => {
        ...
        const matrixEntry = supportMatrixRows.find((row) => normalizeDexLabel(row.dex) === normalizedName || normalizeDexLabel(row.dex) === normalizeDexLabel(name));
    ...
    const activeRouteSymbols = new Set(liveOpportunities.flatMap((opportunity) => [
    ...
    if (killSwitchState.engaged && Date.now() - lastKillSwitchAlertAt > 60_000) {
        ...
        void profitability.sendAlert("kill-switch", killSwitchState.reason ?? "Kill-switch engaged", {
    ...
    return buildDashboardPayload({
        ...
        executionPlan,
        tradeHistory,
        executionIntents: buildExecutionIntentHistory(),
        executionIntentSummary: {
            total: executionIntentHistory.length,
            prepared: executionIntentHistory.filter((intent) => intent.status === "prepared").length,
            submitted: executionIntentHistory.filter((intent) => intent.status === "submitted").length,
            confirmed: executionIntentHistory.filter((intent) => intent.status === "confirmed").length,
            failed: executionIntentHistory.filter((intent) => intent.status === "failed").length,
            active: executionIntentHistory.filter((intent) => intent.status === "prepared" || intent.status === "submitted").length,
        },
        deploymentSafety,
        reconciliation,
        pnlSummary,
...
function getRolloutStageRank(stage) {
    if (stage === "scale") {
        return 2;
    }
    if (stage === "canary") {
...
function getRolloutStageFromRank(rank) {
    if (rank >= 2) {
        return "scale";
    }
    if (rank === 1) {
...
function persistRolloutGovernanceRegistry() {
    writeRolloutGovernanceState(Array.from(rolloutGovernanceRegistry.values()).map((row) => ({
        chain: row.chain,
...
function getOrCreateRolloutGovernanceState(chain, observedStage) {
...
function evaluateRolloutGovernance(chain, observedStage, now) {
...
export async function buildRolloutStatus() {
    ...
    const chainRows = CHAIN_LIST.map((chain) => {
        ...
        const observedStage = scaleReady ? "scale" : canaryReady ? "canary" : "blocked";
...
export function buildStrategyTuningRecommendations() {
    ...
    const currentPrepareCooldown = Number(process.env.EXECUTION_PREPARE_COOLDOWN_MS ?? 1500);
    ...
    if (killSwitch.consecutiveLosses >= 2 || killSwitch.rpcInstabilityEvents >= 2) {
        recommendations.push({
            ...
            recommended: Math.min(15000, Math.round(currentPrepareCooldown * 1.5)),
...
export function buildDeploymentSafetySnapshot() {
    const storeMeta = getExecutionStoreMeta();
    const storeExists = existsSync(storeMeta.filePath);
    const storeStats = storeExists ? statSync(storeMeta.filePath) : undefined;
    const persistenceHealthy = Boolean(storeExists && storeStats && storeStats.isFile() && storeStats.size >= 0);
...
export function buildPostTradeReconciliationSnapshot() {
    const trades = executionLedger.slice();
    const settlements = getSettlementRecords(500);
    const settlementsByHash = new Map(settlements.map((record) => [record.txHash.toLowerCase(), record]));
...
app.get("/strategy/replay", (req, res) => {
    const minutes = Number(req.query.minutes ?? 60);
    res.json(serializeForJson(profitability.getReplaySummary(minutes)));
});
app.post("/strategy/kill-switch/reset", (req, res) => {
...
app.post("/alerts/test", async (req, res) => {
    ...
    const result = await profitability.sendAlert("test", "Habibi Arbitrage Engine test alert", { at: new Date().toISOString() });
    res.status(result.delivered ? 200 : 400).json(serializeForJson(result));
});
app.get("/strategy/tuning/recommendations", (_req, res) => {
    res.json(serializeForJson(buildStrategyTuningRecommendations()));
});
app.get("/execute/recovery/pending", (_req, res) => {
    res.json(serializeForJson(buildRecoveryTicketSnapshot()));
});
app.get("/execute/recovery/worker/status", (_req, res) => {
    res.json(serializeForJson(getRecoveryWorkerStatus()));
});
app.get("/execute/settlements", (req, res) => {
    const limit = Number(req.query.limit ?? 50);
    res.json(serializeForJson({
        records: getSettlementRecords(limit),
    }));
});
app.get("/execute/settlement-queue", (_req, res) => {
    res.json(serializeForJson(buildSettlementQueueSnapshot()));
});
app.get("/execute/settlement-worker/status", (_req, res) => {
    res.json(serializeForJson(getSettlementWorkerStatus()));
});
app.post("/execute/recovery/worker/run", async (req, res) => {
    if (!isExecutionApiAuthorized(req, res)) {
        return;
    }
...
app.post("/execute/settlement-worker/run", async (req, res) => {
    if (!isExecutionApiAuthorized(req, res)) {
        return;
    }
...
app.post("/execute/settlement-queue/hint", (req, res) => {
    if (!isExecutionApiAuthorized(req, res)) {
        return;
    }
...
app.post("/execute/recovery/prepare-auto", async (req, res) => {
    if (!isExecutionApiAuthorized(req, res)) {
        return;
    }
...
app.post("/execute/settle", (req, res) => {
    if (!isExecutionApiAuthorized(req, res)) {
        return;
    }
    if (!isTransactionHash(req.body?.txHash)) {
...
app.get("/ops/metrics", async (_req, res) => {
    const rpcHealth = await buildRpcHealthSnapshot();
...
app.get("/ops/drill/relay-rpc", async (_req, res) => {
    const rpcHealth = await buildRpcHealthSnapshot();
    res.json(serializeForJson(buildRelayRpcDrillStatus({ rpcHealth })));
});
app.get("/ops/readiness-gate", async (_req, res) => {
    const signerPolicy = evaluateSignerPolicy();
    const killSwitch = profitability.getKillSwitchState();
    const canaryValidation = buildCanaryValidationStatus();
    const rpcHealth = await buildRpcHealthSnapshot();
...
app.get("/ops/operator-safety", async (_req, res) => {
    const rpcHealth = await buildRpcHealthSnapshot();
    const signerPolicy = evaluateSignerPolicy();
    const killSwitch = profitability.getKillSwitchState();
    const canaryValidation = buildCanaryValidationStatus();
...
app.post("/rollout/override", async (req, res) => {
    if (!isExecutionApiAuthorized(req, res)) {
        return;
    }
    const chain = typeof req.body?.chain === "string" ? req.body.chain : "";
    if (!CHAIN_LIST.includes(chain)) {
        res.status(400).json(serializeForJson({
            success: false,
    ...
    const state = getOrCreateRolloutGovernanceState(chain, "blocked");
    ...
    rolloutGovernanceRegistry.set(chain, state);
    persistRolloutGovernanceRegistry();
    res.json(serializeForJson({
        success: true,
        chain,
    ...
    try {
        ...
        if (request.privateRelaySubmit) {
            ...
            const submittedRawTransactions = Array.isArray(request.signedTransactions)
                ? request.signedTransactions.filter((transaction) => isHexRawTransaction(transaction))
                : [];
