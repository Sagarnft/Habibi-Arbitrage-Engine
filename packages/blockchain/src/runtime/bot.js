...
function formatOpportunityAmount(value, decimals = 18) {
...
async function updateDashboard(data) {
    try {
        await fetch("http://localhost:4000/dashboard/update", {
            method: "POST",
            headers: {
...
function getAllowedTokens(chain) {
...
const scanRequests = CHAIN_LIST.map((chain) => ({
    chain,
    amountIn: 1000n * 10n ** 6n,
    tokens: getAllowedTokens(chain),
}));
const CHAIN_PRIORITY_RANK = {
    ethereum: 10,
    base: 9,
    arbitrum: 9,
    polygon: 8,
...
export async function startBot() {
    ...
    console.log("Tokens Loaded:", scanRequest.tokens.length);
    while (true) {
        try {
            ...
            if (result.success && result.opportunity) {
                console.log("🔥 Opportunity Found:", result.opportunity);
                ...
                console.log("No Opportunity:", result.error);
                await updateDashboard({
                    status: "running",
        ...
        await new Promise(resolve => setTimeout(resolve, botConfig.intervalMs));
    }
}
